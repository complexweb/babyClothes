...
(function (global, factory) {
    ...
    function scope(target, options, originalOptions) {
        ...
        function addOrigin(base, handleNumber) {
            ...
            handle.setAttribute("aria-label", "Изменение диапазона цены");
...
document.querySelectorAll('.custom-select').forEach((customSelect) => {
  ...
  const customSelectBtnText = customSelectBtn.querySelector('span');
...
let zoomTarget = document.querySelectorAll('.zoomer');
...
if (document.querySelectorAll('.tabs').length > 0) {
    document.querySelectorAll('.tabs').forEach((tab) => {
      let tabBtn = tab.querySelectorAll('.tabs__btn');
      let tabItem = tab.querySelectorAll('.tabs__item');
...
const swiper = new Swiper('.selections__slider', {
  slidesPerView: 1,
  watchOverflow: true,
  spaceBetween: 23,
  navigation: {
...
document.querySelectorAll('.main-search').forEach((mainsearch) => {
  ...
  const mainSearchInputBlock = mainsearch.querySelector('.main-search__input-block');
  const searchLists = mainSearchInputBlock.querySelectorAll('.main-search-suggest__item');

  const openBtn = document.querySelector('.header-bottom__search-mobile-btn');
  const mobileSearch = document.querySelector('.mobile-search');
  const closeBtn = mobileSearch.querySelector('.mobile-search__close');

  const burgerBtn = document.querySelector('.header-bottom__mobile-btn');
  ...
  const catalogOpenBtn = document.querySelector('#header-catalog');
  const catalogMenu = document.querySelector('.catalog-menu');
  const catalogMenuCloseBtn = document.querySelector('.catalog-menu__close');
  const headerSearchMobileBtn = document.querySelector('.header-bottom__search-mobile-btn');

  //catalog mobile filter========
  let hasCatalogFilter = false;
  let catalogFilterOpenBtn;
  let catalogFilterCloseBtn;
  let catalogFilter;

  if (document.querySelector('.catalog__filter')) {
    hasCatalogFilter = true;
    catalogFilterOpenBtn = document.querySelector('.catalog-block__open-filter');
    catalogFilter = document.querySelector('.catalog-pg-filter');
    catalogFilterCloseBtn = document.querySelector('.catalog-pg-filter__close');

    catalogFilterOpenBtn.addEventListener('click', () => {
      catalogFilter.classList.add('_active');
      body.classList.add('_no-scroll');
    })

    catalogFilterCloseBtn.addEventListener('click', () => {
  ...
  if (document.querySelector('.product-page')) {
    let addReviewBtn = document.querySelector('.reviews__add-btn');
    let productBody = document.querySelector('.product-page__body-bottom');
    let reviewsTitle = document.querySelector('.reviews__title');
    let reviewsPopup = document.querySelector('.add-mobile-reviews');
    let reviewsPopupCloseBtn = document.querySelector('.add-mobile-reviews__close');
    let reviews = productBody.querySelectorAll('.reviews__descr-review');
    let showMoreBtn = document.querySelector('.reviews__more-btn');

    let reviewForm = document.querySelector('#add-review');
    let reviewsPopupBody = document.querySelector('#add-mobile-reviews');
    let reviewFormBody = document.querySelector('.reviews__body');

    //if no reviews
    if (document.querySelector('.product-page--no-reviews')) {

      addReviewBtn.addEventListener('click', () => {
        productBody.classList.add('_no-reviews-active');
        if (window.matchMedia('(max-width: 800px)').matches) {
          reviewsPopup.classList.add('_active');
          body.classList.add('_no-scroll');
        }
      })

      reviewsTitle.addEventListener('click', () => {
        productBody.classList.remove('_no-reviews-active');
      })

      reviewsPopupCloseBtn.addEventListener('click', () => {
        productBody.classList.remove('_no-reviews-active');
        reviewsPopup.classList.remove('_active');
        body.classList.remove('_no-scroll');
      });

    }
    //if has reviews
    if (document.querySelector('.product-page--reviews-show')) {
      //start settings
      for (let i = 0; i < reviews.length; i++) {
        if (i > 1) {
          reviews[i].classList.add('_hidden');
        }
      }
      addReviewBtn.classList.add('_hidden');
      //-----------
      showMoreBtn.addEventListener('click', () => {
        productBody.classList.add('_reviews-active');
        addReviewBtn.classList.remove('_hidden');
        for (let i = 0; i < reviews.length; i++) {
          reviews[i].classList.remove('_hidden');
        }
      })

      addReviewBtn.addEventListener('click', () => {
        addReviewBtn.classList.add('_hidden');
        reviewFormBody.classList.add('_visible');
        if (window.matchMedia('(max-width: 800px)').matches) {
          reviewsPopup.classList.add('_active');
          body.classList.add('_no-scroll');
        }
      ...
      reviewsPopupCloseBtn.addEventListener('click', () => {
        ...
        body.classList.remove('_no-scroll');
        addReviewBtn.classList.remove('_hidden');
        reviewFormBody.classList.remove('_visible');
      })

      reviewsTitle.addEventListener('click', () => {
        productBody.classList.remove('_reviews-active');
        addReviewBtn.classList.add('_hidden');
        reviewFormBody.classList.remove('_visible');
        for (let i = 0; i < reviews.length; i++) {
          if (i > 1) {
    ...
    function transferForm() {
      if (window.matchMedia('(max-width: 800px)').matches) {
        reviewsPopupBody.append(reviewForm);
        if (productBody.classList.contains('_no-reviews-active')) {
          reviewsPopup.classList.add('_active');
          body.classList.add('_no-scroll');
        }
        if (productBody.classList.contains('_reviews-active')) {
          if (reviewFormBody.classList.contains('_visible')) {
            reviewsPopup.classList.add('_active');
            body.classList.add('_no-scroll');
          }
        }
        ...
        reviewFormBody.prepend(reviewForm);
        if (productBody.classList.contains('_no-reviews-active')) {
          reviewsPopup.classList.remove('_active');
          body.classList.remove('_no-scroll');
        }
        if (productBody.classList.contains('_reviews-active')) {
          if (reviewsPopup.classList.contains('_active')) {
            reviewsPopup.classList.remove('_active');
            body.classList.remove('_no-scroll');
          }
        }
  ...
  //declaration of variables for mobile catalog accordion 
  const headerCatalogDrops = document.querySelector('#header-catalog-drop');
  const dropBtns = headerCatalogDrops.querySelectorAll('.nav-drop__link');
  const dropBodys = headerCatalogDrops.querySelectorAll('.nav-drop__list');


  searchInput.addEventListener('focus', () => {
    mainSearchInputBlock.classList.add('_active');
    if (burgerBtn.classList.contains('_active')) {
      burgerBtn.classList.remove('_active');
      headerMobileMenu.classList.remove('_active');
      body.classList.remove('_no-scroll');
    }
  })
  ...
  searchLists.forEach((searchItem) => {
    searchItem.addEventListener('click', () => {
      searchInput.value = searchItem.innerText;
    })
  })

  document.addEventListener('click', (e) => {
    if (!e.composedPath().includes(mainsearch) && mainSearchInputBlock.classList.contains('_active')) {
      mainSearchInputBlock.classList.remove('_active');
    }
  })
  ...
  openBtn.addEventListener('click', () => {
    ...
    if (catalogMenu.classList.contains('_active')) {
      catalogMenu.classList.remove('_active');
      headerSearchMobileBtn.classList.remove('_catalog-menu-active');
    }
  })
  ...
  burgerBtn.addEventListener('click', () => {
    if (!burgerBtn.classList.contains('_active')) {
      ...
      headerMobileMenu.classList.add('_active');
      body.classList.add('_no-scroll');
      mobileMenuBody.style.height = `${mobileMenu.offsetHeight + 150}px`;
    } else {
      burgerBtn.classList.remove('_active');
      headerMobileMenu.classList.remove('_active');
      body.classList.remove('_no-scroll');
  ...
  catalogOpenBtn.addEventListener('click', () => {
    ...
    body.classList.add('_no-scroll');
    headerSearchMobileBtn.classList.add('_catalog-menu-active');
  })

  catalogMenuCloseBtn.addEventListener('click', () => {
    catalogMenu.classList.remove('_active');
    body.classList.remove('_no-scroll');
    //close tabs
    headerSearchMobileBtn.classList.remove('_catalog-menu-active');
    for (let i = 0; i < dropBtns.length; i++) {
      if (dropBtns[i].classList.contains('_active')) dropBtns[i].classList.remove('_active');
    }
    for (let i = 0; i < dropBodys.length; i++) {
      if (dropBodys[i].classList.contains('_active')) dropBodys[i].classList.remove('_active');
    }
  })
  ...
  dropBtns.forEach((btn) => {
    btn.addEventListener('click', (e) => {
      if (btn.getAttribute('data-catalog-name')) e.preventDefault();

      if (!btn.classList.contains('_active')) {
        btn.classList.add('_active');
        for (let i = 0; i < dropBodys.length; i++) {
          if (btn.getAttribute('data-catalog-name') === dropBodys[i].getAttribute('data-catalog-drop')) {
            dropBodys[i].classList.add('_active');
          }
        }
      } else {
        ...
        for (let i = 0; i < dropBodys.length; i++) {
          if (btn.getAttribute('data-catalog-name') === dropBodys[i].getAttribute('data-catalog-drop')) {
            dropBodys[i].classList.remove('_active');
          }
        }
      }
    })
  ...
  window.matchMedia('(max-width: 1024px)').addEventListener('change', () => {
    ...
    catalogMenu.classList.remove('_active');
    headerSearchMobileBtn.classList.remove('_catalog-menu-active');
    if (!mobileSearch.classList.contains('_active')) {
      body.classList.remove('_no-scroll');
    }
  })
  ...
  window.matchMedia('(max-width: 800px)').addEventListener('change', () => {
    ...
    if (!burgerBtn.classList.contains('_active') && !catalogMenu.classList.contains('_active') && !headerSearchMobileBtn.classList.contains('_catalog-menu-active')) {
      if (!(hasCatalogFilter && catalogFilter.classList.contains('_active'))) {
        body.classList.remove('_no-scroll');
      }
    }
  })
...
if (document.querySelector('.header')) {
  const headerSearch = document.querySelector('#main-search');
  const mobileSearch = document.querySelector('#mobile-search');
  ...
  const headerTopInner = document.querySelector('#header-top-inner');

  const mobileFooterCatalog = document.querySelector('#mobile-footer-catalog');
  const mobileFooterFavorite = document.querySelector('#mobile-footer-favorite');
  const mobileFooterCart = document.querySelector('#mobile-footer-cart');
  const mobileFooterUser = document.querySelector('#mobile-footer-user');
  const headerCatalog = document.querySelector('#header-catalog');
  const headerNavItemDrop = document.querySelector('#header-nav-item-drop');
  const headerFavorite = document.querySelector('#header-favorite');
  const headerCart = document.querySelector('#header-cart');
  const headerUser = document.querySelector('#header-user');

  const userActionsItemFavorite = document.querySelector('#user-actions-item-favorite');
  const userActionsItemCart = document.querySelector('#user-actions-item-cart');
  const userActionsItemUser = document.querySelector('#user-actions-item-user');

  const headerCatalogDrop = document.querySelector('#header-catalog-drop');
  const catalogMenu = document.querySelector('#catalog-menu');
  const headerNavDropInner = document.querySelector('#header-nav-drop-inner');

  //mobile catalog-pg-filter
  let hasCatalogPgFilter = document.querySelector('#catalog-filter') || false;
  let catalogFilter;
  let catalogPgFilter;
  let catalogInner;
  if (hasCatalogPgFilter) {
    catalogFilter = document.querySelector('#catalog-filter');
    catalogPgFilter = document.querySelector('#catalog-pg-filter');
    catalogInner = document.querySelector('#catalog__inner');
  }

  //800
  if (window.matchMedia('(max-width: 800px)').matches) {
    //header-search
    mobileSearch.prepend(headerSearch);
  } else {
    //header-search
  ...
  if (window.matchMedia('(max-width: 1024px)').matches) {
    //header-nav
    mobileMenu.prepend(headerNav);
    headerNav.after(headerAddress);
    //mobile-footer
    mobileFooterCatalog.prepend(headerCatalog);
    mobileFooterFavorite.prepend(headerFavorite);
    mobileFooterCart.prepend(headerCart);
    mobileFooterUser.prepend(headerUser);
    //mobile-catalog
    catalogMenu.prepend(headerCatalogDrop);
    //mobile-catalog-filter
    if (hasCatalogPgFilter) {
      catalogPgFilter.prepend(catalogFilter);
    }
  } else {
    //header-nav
    headerTopInner.prepend(headerAddress);
    headerAddress.after(headerNav);
    //mobile-footer
    headerNavItemDrop.prepend(headerCatalog);
    userActionsItemFavorite.prepend(headerFavorite);
    userActionsItemCart.prepend(headerCart);
    userActionsItemUser.prepend(headerUser);
    //mobile-catalog
    headerNavDropInner.prepend(headerCatalogDrop);
    //mobile-catalog-filter
    if (hasCatalogPgFilter) {
      catalogInner.prepend(catalogFilter);
    }
  }
...
document.querySelectorAll('.catalog-filter__block').forEach((accordion) => {
  const btn = accordion.querySelector('.catalog-filter__legend');
  const area = accordion.querySelector('.catalog-filter__area');
  const areaInner = accordion.querySelector('.catalog-filter__area-inner');
  let bodyInner;
  if (accordion.querySelector('.custom-scroll')) {
    bodyInner = accordion.querySelector('.custom-scroll');
  }


  let height = areaInner.offsetHeight;
  areaInner.style.height = `auto`;

  if (height <= 200 && bodyInner) {
    bodyInner.classList.add('_no-scroll');
  }

  btn.addEventListener('click', () => {
    if (area.classList.contains('_close')) {
      area.classList.remove('_close');
      btn.classList.remove('_close');
      areaInner.style.height = `auto`;
    } else {
      area.classList.add('_close');
      btn.classList.add('_close');
      areaInner.style.height = '0px';
    }
  })
})
...
if (document.querySelector('.catalog-filter__block')) {
  const resetBtn = document.querySelector('.catalog__reset');

  //range-slider
  if (document.querySelector('.catalog-range__slider')) {
    let catalogFilterRange = document.querySelector('.catalog-range__slider');

    const minInput = document.querySelector('.catalog-range__input-min');
    const maxInput = document.querySelector('.catalog-range__input-max');

    rangeSlider = noUiSlider.create(catalogFilterRange, {
      connect: true,
    ...
    minInput.addEventListener('change', function () {
      rangeSlider.setHandle(0, this.value);

    });
    maxInput.addEventListener('change', function () {
      rangeSlider.setHandle(1, this.value);
    });
...
//catalog-block
if (document.querySelector('.catalog-block')) {
  const catalogBlock = document.querySelector('.catalog-block');
  const catalogBlockBtns = catalogBlock.querySelectorAll('.catalog-block__look-btn');
  const catalogBlockBodyList = catalogBlock.querySelector('.catalog-block__body-list');


  let appearance = localStorage.getItem('catalog-page-apperance') || catalogBlockBtns[0].getAttribute('data-name');
  localStorage.setItem('catalog-page-apperance', appearance);

  //start settings
  for (let i = 0; i < catalogBlockBtns.length; i++) {
    if (catalogBlockBtns[i].getAttribute('data-name') === appearance) {
      catalogBlockBtns[i].classList.add('_active');
      catalogBlockBodyList.classList.add(appearance);
    }
  }

  catalogBlockBtns.forEach((btn) => {
    btn.addEventListener('click', () => {
      if (!btn.classList.contains('_active')) {
        for (let i = 0; i < catalogBlockBtns.length; i++) {
          if (catalogBlockBtns[i] === btn) {
            catalogBlockBtns[i].classList.add('_active');
            catalogBlockBodyList.classList.add(catalogBlockBtns[i].getAttribute('data-name'));
            localStorage.setItem('catalog-page-apperance', btn.getAttribute('data-name'));
          } else {
            catalogBlockBtns[i].classList.remove('_active');
            catalogBlockBodyList.classList.remove(catalogBlockBtns[i].getAttribute('data-name'))
          }
        }
      }
    })
...
//custom scrollbar
let scrollBlocks = document.querySelectorAll('.custom-scroll');
if (window.matchMedia('(min-width: 1025px)').matches) {
  for (let i = 0; i < scrollBlocks.length; i++) {
    new SimpleBar(scrollBlocks[i], {
      autoHide: false,
      scrollbarMinSize: 30,
    });
  }
...
const productMainSlider = new Swiper('.product-main-slider', {
  ...
  thumbs: {
    swiper: {
      el: '.product-min-slider',
      slidesPerView: 4,
      spaceBetween: 4,
      direction: 'horizontal',
      navigation: {
        nextEl: '.product-min-slider__next',
        prevEl: '.product-min-slider__prev'
      },
      breakpoints: {
        1401: {
          slidesPerView: 2,
...
document.querySelectorAll('.tabs').forEach((tab) => {
  ...
  let bodys = tab.querySelectorAll('.tabs__body');

  for (let i = 0; i < btns.length; i++) {
    btns[i].addEventListener('click', () => {
      if (btns[i].hasAttribute('data-name')) {
        btns.forEach((btn) => btn.classList.remove('_active'));
        for (k = 0; k < bodys.length; k++) {
          bodys[k].classList.remove('_active');
          if (bodys[k].classList.contains(btns[i].getAttribute('data-name'))) {
            bodys[k].classList.add('_active');

            if (!btns[i].closest('.tabs__list')) {
              btns[i].classList.add('_active');
            } else {
              btns[i].closest('.tabs__list').firstElementChild.classList.add('_active');
            }
          }
        }
      }
...
if (document.querySelector('.about-page')) {
  const dropBtns = document.querySelectorAll('.about-page__list-inner-item');
  const pageBtnList = document.querySelector('.about-page__list');
  const mobileHiddenBtn = document.querySelector('.about-page__list-item.--mobile-hidden');
  const desctopDropList = document.querySelector('.about-page__list-item-drop');

  if (window.matchMedia('(max-width: 1000px)').matches) {
    mobileHiddenBtn.style.display = 'none';
    for (let i = dropBtns.length; i > 0; i--) {
      pageBtnList.prepend(dropBtns[i - 1]);
    }
  }
  ...
  window.matchMedia('(max-width: 1000px)').addEventListener('change', () => {
    if (window.matchMedia('(max-width: 1000px)').matches) {
      ...
      mobileHiddenBtn.style.display = 'block';
      for (let i = 0; i < dropBtns.length; i++) {
        desctopDropList.append(dropBtns[i]);
      }
    }
  })
}
...
//form nested radio
document.querySelectorAll('.nested-inputs').forEach((body) => {
  const parentInput = body.querySelector('.nested-inputs__parent');
  const childInputs = body.querySelectorAll('.nested-inputs__child');
  const radioInputs = document.querySelectorAll('input[type="radio"]');

  let targetRadio = [];

  for (let i = 0; i < radioInputs.length; i++) {
    if (parentInput.getAttribute('name') === radioInputs[i].getAttribute('name')) {
  ...
  //enebled child Inputs if click parent Input
